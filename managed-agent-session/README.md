...
| 基线日期 | 2026-07-22 |
| 设计范围 | Agent 配置编译、Model/Tool/Skill Manager 绑定与执行、pi-mono-java Runtime 装配、多 Session 创建、Run 执行、审批与恢复 |
...
9. 容器必须在 readiness 之前为部署范围内的每个 Agent 版本完成 `ResolvedAgentRuntime` 装配和注册；Session 创建路径禁止懒加载 Runtime。
...
12. 每个对话仍在打开时创建独立的、有消息状态的 `AgentSession + Agent`；容器启动时不预建空对话。
...
| `ResolvedAgentRuntime` | 容器启动时预装配的 Agent 实例，包含固定提示词和 Manager Manifest | Agent 版本或容器生命周期 | 共享 |
| `AgentSession` | 一个具体对话的运行时对象 | 对话生命周期 | 不共享 |
| `Agent` | pi-mono-java 底层有状态模型循环门面 | 与 Session 相同 | 不共享 |
| `Run` | 一条消息触发的一次模型/工具循环 | 单次执行 | 不共享 |

`ResolvedAgentRuntime` 是“容器启动后已存在的 Agent 实例”的领域表达。它不是当前 `com.campusclaw.agent.Agent`，因为后者持有消息、队列、执行状态和取消状态。前者容器级共享，后者仍与单个 Session 同生命周期。
...
| Agent Runtime | 每个 Session 临时解析 | 容器 readiness 前 eager 装配的不可变 Runtime |
...
`ResolvedAgentRuntime` 在容器 readiness 之前完成创建。`static_system_prompt` 已包含结构化 system、Tool 清单和 Skill 清单；`runtime_tool_set` 已建立按模型 Tool 名称查找的不可变索引。Runtime 不保存模型 Provider、Tool 或 Skill 的业务实现，也不保存 Manager 内部 permission 结构。
...
`bindAgentSkills()` 在省略版本时解析最新版本，但 Agent 版本发布结果必须固定为具体版本。`activate()` 返回 Skill instructions、resources、内容 hash 和已固定的 Tool overlay 引用。Agent Runtime 不直接读取 Skill Manager 的存储。
...
**目标设计**：Agent Runtime 不在第一个 Session 到来时创建。容器启动阶段必须先完成所有必需 Runtime 的装配，然后才进入 `ACCEPTING_TRAFFIC`。
...
function bootstrapAgentRuntimes():
    ...
    for config in configs:
        runtime = assembleAgentRuntime(config)
...
装配成功后，由 `AgentRuntimeRegistry` 按 `agent_id + version` 注册不可变 Runtime。启动阶段完成后，该 Registry 快照只读。

### 9.3 禁止 Session 路径懒加载

`AgentRuntimeRegistry` 是已预装配 Runtime 的查询容器，不是 Runtime Factory。它不得持有 `AgentConfigRepository`、`AgentRuntimeAssembler` 或者任何 `Supplier<ResolvedAgentRuntime>`，也不得使用 `computeIfAbsent()` 在 Session 请求线程中装配 Runtime。

```text
interface AgentRuntimeRegistry:
    state() -> BOOTSTRAPPING | READY | FAILED

    requirePreloaded(agent_id, requested_version)
        -> ResolvedAgentRuntime
```

`requested_version` 存在时只允许精确查找。省略版本时，Registry 使用启动 Deployment Plan 中已固定的 active version，不临时访问 AgentConfigRepository 查询 latest。Runtime 不存在时直接返回 `AGENT_RUNTIME_NOT_PRELOADED`。

配置升级默认通过重启容器加载新 Deployment Plan。如果后续需要热更新，必须提供显式的管理面 `deployAgentVersion()`：它在请求路径之外完成新 Runtime 装配和原子发布，不就地修改已存在 Runtime，也不改变已有 Session 固定的版本。
...
容器启动的 Runtime 装配阶段生成 `static_system_prompt`：
...
当前 `AgentSession.resolveModel(String)` 支持一个字符串和模糊匹配。目标 `model[]` 必须在容器启动的 Runtime 装配阶段由 Model Manager 解析为固定 Model Manifest。Session 创建时由 Model Manager 从该固定 Manifest 中选择 `ModelBindingRef`，不得重新查询模型配置或扩展候选集。
...
function createManagedSession(
    ...
    runtime = AgentRuntimeRegistry.requirePreloaded(
        agentId,
        requestedVersion
    )
...
`createManagedSession()` 不调用模型，也不读取 AgentConfig、不调用 `bindAgentModels()`、`bindAgentTools()` 或 `bindAgentSkills()`，不重新编译静态 system prompt。后续 `session.prompt()` 才进入支持 `model_invoker + model_tools + tool_call_dispatcher + skill_call_dispatcher` 的 AgentLoop。
...
模型请求的 `tools` 来自当前 Session 的有效 Tool Set：基础 Agent Tool，加上已经激活 Skill 的固定 Tool overlay。Model Manager 只负责把这批 Schema 发送给具体 Provider。
...
```text
function resumeApproval(sessionId, runId,
                        approvalId, decision):
...
用户拒绝、审批过期属于结构化 ToolResult，模型可以看到该结果并调整方案；审批记录不存在、已消费、参数 hash 不匹配或 actor 不匹配属于协议错误，不执行 Tool。
...
Runtime 和 Manager 必须能够按固定 Manifest 恢复；不能按名称静默绑定到最新 Tool 或 Skill。容器启动时，`AgentDeploymentPlan` 必须包含所有需要继续恢复的 Agent 历史版本；未预加载的版本不得在恢复路径临时装配。
...
| Session 请求未预加载的 Agent 版本 | 返回 `AGENT_RUNTIME_NOT_PRELOADED` | 禁止在请求路径懒加载 |
...
| Tool Manager 要求审批 | Run 保存 Checkpoint 后进入 `WAITING_APPROVAL` | 权限由 Manager 权威决定且不阻塞执行线程 |
...
| `modules/managed-agent-runtime` | AgentRuntimeAssembler、Registry、Bootstrap、三个 Manager Gateway Client、Session/Run 快照 |
| `modules/coding-agent-cli` | CLI/Server 入口、Legacy Session 路径和 Managed Session API；不再在 Managed Session 中解析 Manager 配置 |
...
| `AgentSession.java` | Legacy `initialize(SessionConfig)` 保留；新增独立 `ManagedAgentSessionFactory`，避免在一个初始化方法中混合两套生命周期 |
| `SessionPool.java` | Managed key 改为 `conversation_id + agent_id + agent_version`，通过 Registry 和 Managed Factory 创建；禁止调用 Tool/Skill refresh |
| `SessionManager.java` | 除消息 JSONL 外持久化 `SessionSnapshotV2` 和 `RunCheckpointV1`，支持 `WAITING_APPROVAL` 恢复 |
...
com.campusclaw.managed.runtime.ResolvedAgentRuntime
...
- 确保 Registry 不持有 Repository、Assembler 或 Runtime Supplier。
- 实现 Model Manager 映射。
...
- Session 创建不调用 Config Repository、`bindAgentModels()`、`bindAgentTools()` 或 `bindAgentSkills()` 的边界测试。
...
- `ApprovalRequired` 不阻塞线程、Checkpoint 原子持久化、resume 防重放测试。
...
6. Tool/Skill Manifest 的保留期限是否至少覆盖所有关联 Session。
