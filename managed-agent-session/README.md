...
| 基线日期 | 2026-07-22 |
| 设计范围 | 纯 ToB Agent 配置发布、远程 Model/Tool/Skill Manifest 装配、容器级 Agent 实例和 Runtime Revision 同步、pi-mono-java Server WebSocket 中的多 Session、结构化调用、审批与恢复 |
...
2. Agent Runtime 不解析 Tool 的 `enabled`、`permission` 和默认/逐项覆盖规则；这些规则由 Tool Manager 在 Agent 版本发布阶段解析并固化到 Tool Manifest。
3. Agent Runtime 不解析 Skill 版本或加载 Skill 正文；这些工作由 Skill Manager 在 Agent 版本发布阶段完成并固化到 Skill Manifest。
...
9. Control Plane 发布 `PublishedAgentRevision`，其中固定 AgentConfig 及 `model_manifest_id + tool_manifest_id + skill_manifest_id`；容器副本只能按这些精确 ID 获取 Manifest，不得重新按 name 或 latest 绑定。
10. 容器必须在 readiness 之前为部署范围内的每个 Agent 版本创建 `ManagedAgentInstance`，并发布第一个 `ResolvedAgentRuntimeRevision`；Session 创建路径禁止懒加载 Runtime。
11. Managed 路径是纯 ToB 运行模式：Agent Runtime 不扫描或读取本地 `SYSTEM.md`、`AGENTS.md`、`SKILL.md`、Prompt Template、Extension 或本地 Tool 文件，也不把 cwd 当成资源来源。
12. 目标执行路径使用 `ModelManagerInvoker` 和统一的 `CallDispatcher`。只有 Model Manager 返回的规范化结构化 Call 或用户显式 `/skill:name` 命令可以触发调用；普通文本、ToolResult 文本和 Skill instructions 永远不作为调用发现来源。
13. `RuntimeCallableRegistry` 统一保存 `TOOL` 与 `SKILL_ACTIVATION` 两类模型可调用项。普通 ToolCall 转发 Tool Manager；Runtime 保留的 `activate_skill` Call 转发 Skill Manager。
14. Tool Manager 根据 Manifest 中固定的 effective permission 返回 `Completed`、`ApprovalRequired`、`Denied` 或 `Failed`；Agent Runtime 不读取 permission，只根据结果类型暂停或继续。
15. 管理面更新 Agent、Tool、Skill、Model 或 Settings 时，Control Plane 先发布完整的新 `PublishedAgentRevision`，`AgentRuntimeReconciler` 再按精确 Manifest 重建并原子替换 `activeRevision`。
16. 新建 Session 获取当前 active Revision；已存在 Session 默认固定创建时的 Revision，不在 Run 或审批中途热替换。
17. 每个对话仍在打开时创建独立的、有消息状态的 `AgentSession + Agent`；容器启动时不预建空对话。
18. 创建 `ManagedAgentInstance`、`AgentSession` 和 `Agent` 是普通 Java 对象构造；虚拟线程只承载一次 Run 中的远程模型、Tool 和 Skill 调用，对象本身不“属于”某个虚拟线程。
...
| `PublishedAgentRevision` | Control Plane 发布的不可变 AgentConfig 与三个精确 Manifest ID 的组合 | 发布版本生命周期 | 共享 |
...
| `RuntimeCallableRegistry` | 固定模型调用名到 Tool 或 Skill 激活目标的不可变索引 | Runtime Revision 生命周期 | 共享 |
| `PinnedSessionResourceView` | Session 对固定 prompt、Model、Tool、Skill 与调用注册表的只读视图 | Session 生命周期 | 不跨 Session 共享 |
...
Model Manager、Tool Manager 和 Skill Manager 是独立于 Agent Runtime 的模块。本文只定义它们与发布层、装配层、Session 执行层之间的契约，不设计 Manager 内部如何连接模型 Provider、MCP Server、builtin tool、Skill Artifact Store 或审批 UI。

**产品约束**：Managed 路径是纯远程资源模式。当前源码中的本地 `ResourceLoader`、`DefaultToolCatalog`、`SkillLoader`、`SkillExpander`、上下文文件加载和本地 Extension 仅用于说明现状或保留在 Legacy 路径，不得成为 Managed Runtime 的后备数据源。若未来需要企业知识或项目上下文，应新增独立的远程 Context Manager 契约，不得恢复 cwd 文件扫描。
...
| Tool 执行入口 | `AgentTool.execute()` | `CallDispatcher` 按 Registry 解析后转发 `ToolManager.invoke()` |
...
| 资源发现 | Session 按 cwd 扫描文件与进程内注册项 | 容器启动按 `PublishedAgentRevision` 获取三个精确远程 Manifest |
| 调用发现 | `AgentTool` 本地实现与 `/skill:name` 文件展开 | 只接受 Model Manager 规范化 Call 或显式 `/skill:name`；不解析普通文本 |
| Session 资源视图 | Session 自行加载和刷新 | `PinnedSessionResourceView` 只读引用容器已装配 Revision，不发远程请求也不访问文件 |
...
因此，不能直接把 Agent 配置塞入现有 `SessionConfig.customPrompt`，不能把 Manager Tool 映射成永久的本地实现，也不能把 Manager Skill 映射为当前依赖本地 Path 的 `Skill`。Managed 路径也不能在 Manager 不可用时回退到本地文件或本地 ToolCatalog；这会让同一发布版本在不同副本上产生不同能力集合。
...
### 8.1 发布对象、容器级 Agent 实例与 Runtime Revision

```text
record PublishedAgentRevision:
    ...
    settings_snapshot: RuntimeSettingsSnapshot
    desired_runtime_revision_id: string
...
record ResolvedAgentRuntimeRevision:
    ...
    callable_registry: RuntimeCallableRegistry
...
record PinnedSessionResourceView:
    runtime_revision_id: string
    static_system_prompt: string
    model_candidates: list<ModelDescriptor>
    model_tools: list<ModelToolDescriptor>
...
`PublishedAgentRevision` 是 Control Plane 对外发布的唯一装配输入。Control Plane 在发布阶段把 Agent 配置交给三个 Manager 完成 name 查找、默认值合并、权限计算、Skill 递归依赖解析和版本固定，然后保存三个不可变 Manifest ID。各容器副本不得独立调用 bind 接口，也不得按 `latest` 解析资源。

完整 AgentConfig 保留在发布对象中用于审计、hash 校验和编译 `system/use_cases/metadata`；Runtime Assembler 不遍历或解释其中的 `model/tools/skills` 字段，能力集合以三个 Manifest 为权威值。

`PinnedSessionResourceView` 由 `ResolvedAgentRuntimeRevision` 在内存中创建，只包含 Session 需要的只读数据。创建 Session 时不调用 Manager 的 Manifest API、不扫描文件、不刷新 Tool/Skill，也不复制可变对象。

### 8.2 Control Plane 发布与远程 Manifest 加载

```text
interface AgentControlPlane:
    publishAgentRevision(agent_config)
        -> PublishedAgentRevision

    getPublishedRevision(publication_id)
        -> PublishedAgentRevision

interface RemoteManifestLoader:
    loadPublishedRevision(publication_id)
        -> PublishedAgentResourceBundle

record PublishedAgentResourceBundle:
    publication: PublishedAgentRevision
    model_manifest: ModelManifest
    tool_manifest: ToolManifest
    skill_manifest: SkillManifest
    tool_overlay_manifests:
...
```text
Agent Control Plane validates AgentConfig
    -> Model Manager publishes ModelManifest
    -> Tool Manager publishes ToolManifest
    -> Skill Manager publishes SkillManifest
       and obtains pinned Tool overlays from Tool Manager
    -> Control Plane atomically publishes PublishedAgentRevision
```

容器中的 `RemoteManifestLoader` 只执行精确读取和交叉校验：

```text
function loadPublishedRevision(publicationId):
    publication = AgentControlPlane
        .getPublishedRevision(publicationId)

    manifests = parallel(
        ModelManager.getManifest(
            publication.model_manifest_id),
        ToolManager.getManifest(
            publication.tool_manifest_id),
        SkillManager.getManifest(
            publication.skill_manifest_id)
    )

    require every manifest.agent_key
        == publication.agent_key
    require every returned manifest id
        == its published id
    require canonicalHash(publication.agent_config)
        == publication.agent_config_hash

    overlayIds = manifests.skill_manifest.skills
        .map(skill -> skill.tool_overlay_manifest_id)
        .removeEmpty()

    toolOverlays = ToolManager.getExactManifests(
        overlayIds
    )

    require toolOverlays exactly cover overlayIds

    return immutable bundle with toolOverlays
```

这里没有 cwd、agentDir、`SKILL.md`、`AGENTS.md`、本地 Prompt Template 或 Extension 的查找步骤。Manager 暂时不可用时加载失败，容器不使用本地缓存内容冒充发布版本；如需可用性缓存，只能缓存按 Manifest ID 校验过的远程不可变响应，并且缓存策略属于 Gateway 基础设施，不改变语义。

### 8.3 Model Manager 契约
...
interface ModelManager:
    publishManifest(agent_key, raw_model_refs,
                    publication_context)
        -> ModelManifest
...
Model Manager 负责 Provider 绑定、API Key、base URL、headers、协议字段映射、限流、重试和实际模型请求。Agent Runtime 只持有公开 Descriptor 与不透明 `binding_id`。Session 创建时从已固定的 `ModelManifest.models` 按本地确定性 `ModelSelectionPolicy` 选择 Binding，不再次查询 Model Manager；实际 Run 才把固定 Binding 发送给 Model Manager。

当前 `modules/ai` 的 Provider Adapter、认证和模型执行代码应迁入独立 Model Manager 服务；Agent Runtime 侧只使用 `ModelManagerGateway`。当前 `StreamFunction` 是第一阶段可复用的远程适配点，但最终接口不应继续要求 Runtime 传入包含 provider、baseUrl、headers、apiKey 的完整 `Model`，也不提供进程内 Provider 回退。
...
interface ToolManager:
    publishManifest(agent_key, raw_toolsets,
                    publication_context) -> ToolManifest
    getManifest(manifest_id) -> ToolManifest
    invoke(invocation_request) -> ToolInvocationResult
    resume(approval_decision) -> ToolInvocationResult
```

`publishManifest()` 内部负责：
...
record ModelSkillDescriptor:
    ...
    child_skill_binding_ids: list<string>

interface SkillManager:
    publishManifest(agent_key, raw_skill_refs,
                    publication_context) -> SkillManifest
    getManifest(manifest_id) -> SkillManifest
...
record ActivatedSkill:
    ...
    version: string
    instructions: string
    resource_refs: list<RemoteResourceRef>
    content_hash: string
    tool_overlay_manifest_id: optional string
```

Skill 引用省略版本时，只能在 Control Plane 发布阶段把 latest 解析一次并固定为具体版本；容器只读取已经发布的 Skill Manifest。`activate()` 返回 Skill instructions、远程资源引用、内容 hash 和已固定的 Tool overlay 引用。Agent Runtime 不直接读取 Skill Manager 的存储或本地文件。

如果某种“Skill”本身执行有副作用的业务动作，应把该动作暴露成 Tool；Skill 仍表示指令和资源，避免一个概念同时承担提示词和远程动作两种语义。

### 8.6 RuntimeCallableRegistry
...
record RuntimeCallableDescriptor:
    model_name: string
    description: string
    input_schema: JsonSchema
    target_kind: CallTargetKind
    target_manifest_id: string
    target_binding_id: optional string

record RuntimeCallableRegistry:
    by_model_name:
        map<string, RuntimeCallableDescriptor>
```

Registry 由一个 Tool Manifest 和 Runtime 保留的 `activate_skill` Descriptor 构建。普通 Skill 不直接伪装为一个任意 Tool；它们通过 `activate_skill({skill_name})` 或用户显式 `/skill:name` 激活。所有模型可调用项统一进入请求的 `tools` 字段，`target_kind` 仅存在于 Runtime 内，不发送给模型。

`activate_skill` 的模型 Schema 由固定 Skill Manifest 生成，至少包含 `skill_name`，并通过 `enum` 限制为当前 Agent 可激活的 Skill name。Skill 清单中的 description 帮助模型选择，Schema 负责约束调用参数；二者来自同一个 Skill Manifest。

```text
function buildCallableRegistry(toolManifest, skillManifest):
    entries = toolManifest.tools.map(tool ->
        RuntimeCallableDescriptor(
            model_name = tool.model_name,
            description = tool.description,
            input_schema = tool.input_schema,
            target_kind = TOOL,
            target_manifest_id = toolManifest.manifest_id,
            target_binding_id = tool.binding_id
        )
    )

    if skillManifest.skills is not empty:
        entries.add(buildActivateSkillDescriptor(
            skillManifest
        ))

    require unique model_name across entries
    return immutable index(entries)
```
...
Agent 启动范围来自 Control Plane 的 `AgentDeploymentPlan`，不在 AgentConfig 中新增一个 Runtime 自行解析的 `enabled` 字段。Deployment Plan 必须给出需要预装配的 `publication_id`；该 ID 间接固定 AgentKey 和三个 Manifest。
...
function bootstrapAgentRuntimes():
    ...
    publicationIds = AgentDeploymentPlan
        .listRequiredPublicationIds()

    for publicationId in publicationIds:
        bundle = RemoteManifestLoader
            .loadPublishedRevision(publicationId)

        revision = assembleAgentRuntimeRevision(bundle)

        instance = ManagedAgentInstance.bootstrap(
            bundle.publication.agent_key,
            revision
        )
...
```text
function assembleAgentRuntimeRevision(bundle):
    publication = bundle.publication
    agentConfig = publication.agent_config
    settingsSnapshot = publication.settings_snapshot
    modelManifest = bundle.model_manifest
    toolManifest = bundle.tool_manifest
    skillManifest = bundle.skill_manifest
    ...
    require agentKey == publication.agent_key
    require modelManifest.manifest_id
        == publication.model_manifest_id
    require toolManifest.manifest_id
        == publication.tool_manifest_id
    require skillManifest.manifest_id
        == publication.skill_manifest_id
    ...
    callableRegistry = buildCallableRegistry(
        toolManifest,
        skillManifest
    )
    ...
    return ResolvedAgentRuntimeRevision({
        ...
        runtime_revision_id:
            publication.desired_runtime_revision_id,
...
装配完成后必须校验实际 Manager Descriptor 与 `PublishedAgentRevision` 中的 Manifest ID 一致。`desired_runtime_revision_id` 是 Control Plane 对 AgentConfig hash、Settings snapshot 和三个 Manifest ID 生成的稳定目标 ID；`runtime_content_hash` 是容器对最终 Manifest、Descriptor、调用注册表和提示词生成的内容 hash。任一 Manager 返回非目标 Manifest 时装配失败，不允许发布近似结果。
...
Manager 不直接持有或修改 `ManagedAgentInstance`。管理面或 Control Plane 先保存新资源版本，通过依赖索引找到受影响的 AgentKey，为每个 AgentKey 重新发布完整 `PublishedAgentRevision`，再发送只包含 `publication_id` 的 reconcile 指令。
...
function onManagedResourceChanged(change):
    ...
    for agentKey in affectedAgentKeys:
        publication = AgentPublicationService
            .publishNextRevision(
                agentKey,
                persistedVersion
            )
...
function reconcileAgentRuntime(request):
    ...
    bundle = RemoteManifestLoader.loadPublishedRevision(
        request.publication_id
    )

    instance.markReconciling(
        bundle.publication.desired_runtime_revision_id
    )

    oldRevision = instance.activeRevision.get()

    try:
        nextRevision = assembleAgentRuntimeRevision(bundle)
...
`AgentRuntimeRegistry` 是已预创建 `ManagedAgentInstance` 的查询容器，不是 Runtime Factory。它不得在 Session 请求线程中调用 Agent Control Plane、`RemoteManifestLoader`、`AgentRuntimeAssembler` 或任何 Manager Manifest API，也不得使用 `computeIfAbsent()` 装配 Runtime。
...
`requested_version` 存在时只允许精确查找。省略版本时，Registry 使用 Deployment Plan 中已固定的 active Agent version，不临时访问 Control Plane 查询 latest。Agent 实例或指定 Revision 不存在时直接返回 `AGENT_RUNTIME_NOT_PRELOADED` 或 `AGENT_RUNTIME_REVISION_NOT_RETAINED`。
...
- 由服务端可信时钟生成的当前时间。
- 由认证层提供的 tenant、user 和环境。
- WebSocket 或业务网关注入的、已经授权的会话属性。

Managed 模式不访问 cwd，也不加载本地 `SYSTEM.md`、`APPEND_SYSTEM.md`、`AGENTS.md`、`CLAUDE.md`、Prompt Template、Extension 或其他项目上下文文件。Agent 身份和静态能力只来自 `PublishedAgentRevision`。若未来需要动态企业知识，应通过单独的远程 Context Manager 或 Tool 接入，并固定其授权与版本语义。
...
**架构变更**：Agent Runtime 持有模型可见的 `RuntimeCallableRegistry` 和统一 `CallDispatcher`，不再要求每个 Tool 实现 `AgentTool.execute()`，也不从本地 Skill 文件推断激活入口。Registry 是当前 Runtime Revision 固定的调用清单，不是文件目录或开放的服务发现中心。

![远程能力发现与结构化调用分发](managed_callable_discovery_dispatch.svg)

[PlantUML 源码](diagram.puml#L176)

发现与调用是两个不同阶段：

| 能力 | 容器如何发现 | 模型如何发现 | 如何触发执行 |
|---|---|---|---|
| Tool | `RemoteManifestLoader` 按 `tool_manifest_id` 获取 Descriptor | system prompt Tool 清单，以及请求 `tools` 中的 name、description、schema | Model Manager 返回同名结构化 Call Block，Dispatcher 转发 Tool Manager |
| Skill | `RemoteManifestLoader` 按 `skill_manifest_id` 获取 Descriptor | system prompt Skill 清单，以及 `activate_skill.skill_name` 枚举 | 模型调用 `activate_skill` 或用户显式 `/skill:name`，Dispatcher 转发 Skill Manager |

Runtime 在装配时完成“当前 Agent 有什么能力”的发现；每轮 Run 只判断“模型是否产生了一个已注册的结构化调用”。它不在对话文本中重新做服务发现。
...
interface CallDispatcher:
    dispatch(normalized_call, session_execution_context)
        -> CallDispatchResult
...
Model Manager 必须把 Anthropic、OpenAI、Mistral 等 Provider 的差异归一化为结构化 Call Block：

```text
record NormalizedCall:
    call_id: string
    name: string
    arguments: JsonValue

sealed NormalizedAssistantBlock:
    TextBlock(text)
    ReasoningBlock(content)
    CallBlock(NormalizedCall)
```

只有 `CallBlock` 进入 `CallDispatcher`。`TextBlock` 中即使出现 `tool(name=...)`、JSON 或 `/skill:x` 字样，也只是文本；ToolResult、Skill instructions 和日志同样不得被二次扫描为调用。用户 `/skill:name` 是 WebSocket 命令层显式转换出的 `SKILL_ACTIVATION` 请求，不是对任意自然语言的解析。
...
function invokeModel(agentContext):
    llmContext = Context(
        ...
        agentContext.callable_registry.asModelTools()
    )
...
function runCallPhase(callBlocks, agentContext):
    results = []

    for call in callBlocks:
        results.append(
            agentContext.call_dispatcher.dispatch(
                call,
...
```text
class ManagerBackedCallDispatcher
    ...
    dispatch(call, context):
        descriptor = callable_registry
            .findByModelName(call.name)

        if descriptor is null:
            return callError(
                call,
                "CALL_NOT_AVAILABLE_TO_AGENT"
            )

        if descriptor.target_kind == TOOL:
            return dispatchTool(
                descriptor,
                call,
                context
        ...
        if descriptor.target_kind == SKILL_ACTIVATION:
        ...
        return callError(call, "UNKNOWN_CALL_TARGET")

    dispatchTool(descriptor, call, context):
        return tool_manager.invoke({
            manifest_id: descriptor.target_manifest_id,
            binding_id: descriptor.target_binding_id,
            ...
            arguments_hash: canonicalHash(call.arguments),
            execution_context:
                context.tool_execution_context
        })

    activateSkill(descriptor, call, context):
        skill = resolvePinnedSkill(
            descriptor.target_manifest_id,
            call.arguments.skill_name
        )
        return skill_manager.activate({
            manifest_id: descriptor.target_manifest_id,
...
Agent Runtime 只根据已固定 Registry 解析模型调用名和目标类型，不解析 permission，也不执行 Tool 或 Skill。Tool Manager 使用固定 Manifest 完成 schema 校验、权限审批、实际执行和结果标准化；Skill Manager 使用固定 Skill Manifest 完成激活。
...
### 11.3 Tool 调 Tool 与 Skill 调 Skill

Agent Runtime 不从调用结果中“发现下一次调用”：

- Tool 需要另一个 Tool 时，优先由 Tool Manager 在其内部完成组合；如果需要模型决定下一步，ToolResult 返回模型，模型再产生新的结构化 Call Block。
- ToolResult 中包含 Tool 名称、JSON 或命令文本时，Runtime 不自动执行。
- Skill instructions 需要另一个 Skill 时，必须让模型显式调用 `activate_skill`，或由 Skill Manager 在发布阶段把固定子 Skill 关系编译为允许的激活目标。
- Skill Manager 在发布阶段校验父子依赖、固定版本、循环依赖和最大深度；Runtime 激活时再次校验目标 binding 属于当前 Manifest 允许的依赖图。
- 同一 Session 的 active Skill 栈保存 `binding_id`，Runtime 对重复激活、循环激活和深度越界返回结构化错误。

```text
Tool A result
    -> append structured result
    -> next model turn
    -> optional new CallBlock for Tool B

Skill A instructions
    -> append activation result
    -> next model turn
    -> optional activate_skill CallBlock for Skill B
```

### 11.4 `ManagerBackedAgentTool` 兼容方案
...
该适配器不包含具体 Tool 实现，但它仍然保留了“每个 Tool 映射为一个 `AgentTool`”的现有类型约束。它不是目标架构的必需对象，应在统一 `CallDispatcher` 落地后移除。

### 11.5 Anthropic Schema 完整性缺口
...
**目标设计**：Model Manager 内部的 Provider Adapter 必须尽可能无损传递 Tool Manager 固定的 Schema。如果目标 Provider 不支持某个 JSON Schema 特性，Model Manifest 发布或 Runtime 装配应失败并报告不兼容关键字，不得静默丢弃。
...
### 11.6 Skill Descriptor 到当前 Skill 流程
...
function loadManagedSkill(skillId, runtime, sessionContext):
    ...
    require activated.content_hash
        == allowed.content_hash

    return activated
```

接入方式固定为：模型请求中注册一个 Runtime 保留的 `activate_skill` 调用 Schema，由统一 `CallDispatcher` 转发 Skill Manager；用户 `/skill:name` 使用同一 Dispatcher。它是 Skill 激活协议，不进入 Tool Manager 的业务 Tool permission 计算。Managed 路径不允许把 Artifact 物化为本地 `Skill(Path)` 后交给现有文件加载器。
...
SystemPromptBuilder.appendSessionContext({
    ...
    trusted_session_context: sessionContext
})
```

`buildManagedStatic()` 取代硬编码 `BASE_PROMPT`，但继续复用当前 Tool/Skill 清单格式。`tools` 和 `skills` 只接收 Runtime 装配时已固定的 Descriptor 快照；Tool 清单渲染 name 和 description，而完整 callable schema 由 `RuntimeCallableRegistry.asModelTools()` 传递到模型请求。`appendSessionContext()` 只处理认证层和业务网关注入的可信会话属性，不调用 Manager、不访问 cwd 或项目文件。
...
function createManagedSession(
    ...
    runtime = runtimeLease.revision
    resources = PinnedSessionResourceView.from(runtime)
    ...
    modelSelection = ModelSelectionPolicy.select(
        resources.model_candidates,
        boundContext
    )
    ...
    callDispatcher = ManagerBackedCallDispatcher({
        callable_registry:
            resources.callable_registry,
    ...
    finalSystemPrompt =
        SystemPromptBuilder.appendSessionContext({
            static_prompt: resources.static_system_prompt,
            trusted_session_context: boundContext
        })

    sessionState = SessionStore.createOrOpen(sessionId)
    ...
    session = AgentSessionFactory.createManaged({
        ...
        resources: resources,
        model_tools:
            resources.callable_registry.asModelTools(),
        call_dispatcher: callDispatcher,
        session_state: sessionState
    })

    restoredMessages = sessionState.loadMessages()
...
`RuntimeLease` 和 `PinnedSessionResourceView` 由 `AgentSession` 持有并在 Session 关闭或驱逐时释放。Registry 只有在某个历史 Revision 已不是 active、没有 Session Lease、没有待恢复 Snapshot 且已满足保留期时，才能驱逐它以及对应 Manager Manifest。
...
`createManagedSession()` 不调用模型，不访问 Agent Control Plane 或 Manager Manifest API，不读取 AgentConfig 或本地文件，也不重新编译静态 system prompt。模型 Binding 从固定 `model_candidates` 中按本地确定性策略选择；WebSocket 后续收到 `prompt` 才进入支持 `model_invoker + callable_registry + call_dispatcher` 的 AgentLoop。
...
在过渡模式中，Session Factory 可把 `RuntimeCallableRegistry` 中 `target_kind == TOOL` 的条目物化为 `List<ManagerBackedAgentTool>` 传入现有 `agent.setTools()`，并把 `activate_skill` 物化为一个通用 `ManagedSkillActivationTool`。这只用于复用现有 `AgentLoop` 的 ToolCall 协议，不允许适配器访问本地文件，也不应把兼容形态写入 Manager 契约。
...
| 组件 | Run 中负责 | Run 中不负责 |
|---|---|---|
| Agent Runtime | 组装每轮请求、消费规范化流、追加消息、按 Registry 分发结构化 Call、暂停与恢复 | Provider 认证、解析普通文本寻找调用、Tool permission 判断、Tool/Skill 业务执行 |
| Model Manager | 根据固定 Binding 执行模型并返回包含 `CallBlock` 的规范化流 | 修改 Session 消息、擅自改变 Tool 清单 |
...
[PlantUML 源码](diagram.puml#L448)

`WAITING_APPROVAL` 是可持久化的暂停状态，不占用虚拟线程。Agent Runtime 不读取 `permission` 决定是否进入该状态；只有 Tool Manager 返回 `ApprovalRequired` 时才进入。

第一阶段 Managed 模式固定为同一批结构化 Call 串行分发。这样在某个 Tool 调用等待审批时，可以精确保存 `next_call_index`；后续如需并行，必须先定义部分成功、多个同时审批和取消传播语义。
...
function run(session, userMessage):
    ...
    while not run.cancelled:
        request = ModelInvocationRequest({
            ...
            tools: session.callable_registry.asModelTools(),
        ...
        // Only structured CallBlock values are calls.
        calls = assistant.normalized_blocks
            .filter(block -> block is CallBlock)
        if calls is empty:
        ...
        for index from 0 to calls.size - 1:
            ...
            result = CallDispatcher.dispatch(
                call,
...
一次 Run 在开始时通过 Session 的 `RuntimeLease` 固定 Revision，后续每个模型 Turn、ToolCall、Skill 激活和审批恢复都使用该身份。模型请求的 `tools` 来自当前 Session 的 `RuntimeCallableRegistry`：基础 Agent Tool、Runtime 保留的 `activate_skill`，以及已经激活 Skill 的固定 Tool overlay。Model Manager 负责把这批 Schema 发送给具体 Provider，并把 Provider 的 Call 事件归一化为 `CallBlock`；Agent Runtime 不解析自然语言寻找调用。

### 13.4 Tool 调用与权限判断

```text
Model Manager returns normalized CallBlock(name, arguments)
    -> CallDispatcher resolves pinned TOOL descriptor
...
Agent Runtime 的调用注册表检查和 Tool Manager 的 Binding 检查分别回答：
...
模型产生 Runtime 保留的 `activate_skill` Call Block，或 WebSocket 命令层收到用户显式 `/skill:name` 时，统一 `CallDispatcher` 使用固定 Skill Binding 调用 Skill Manager。Skill Manager 返回固定版本的 instructions、远程 resources 引用、content hash 和 Tool overlay 引用。

Agent Runtime 校验返回值与 Runtime 中固定 Descriptor 一致，然后：

1. 把 Skill instructions 作为激活结果加入当前消息上下文。
2. 将固定 Tool overlay 作为新的 `TOOL` 条目合并到 Session 的有效 `RuntimeCallableRegistry`，并检查名称冲突。
...
function restoreManagedSession(sessionId, context):
    ...
    require runtime.runtime_revision_id
        == snapshot.runtime_revision_id
    require runtime.publication_id
    ...
    require canonicalHash(runtime.callable_registry)
        == snapshot.callable_registry_hash
...
| PublishedAgentRevision 或任一精确 Manifest 不存在 | 容器启动/Reconcile 失败，旧 Revision 继续服务 | 禁止运行副本自行重新绑定 |
| Manifest 的 AgentKey、ID 或 hash 与发布对象不一致 | Runtime 装配失败 | 防止跨 Agent 或供应链替换 |
...
| `activate_skill` 与业务 Tool 名称冲突 | Runtime 装配失败 | RuntimeCallableRegistry 必须唯一解析 |
...
| 普通文本包含 Tool/Skill 名称或 JSON | 仅作为文本追加，不触发调用 | 只有规范化 Call Block 或显式 Skill 命令可调用 |
| Managed 路径尝试访问 cwd 或本地资源文件 | 启动或测试失败 | 纯 ToB 路径禁止本地资源回退 |
...
| `PublishedAgentRevision + RemoteManifestLoader` | 架构变更 | 发布面先固定三个 Manifest，运行副本只按精确 ID 加载，不在本地重新绑定 |
...
| `RuntimeToolSet` | 架构变更 | 模型 Tool Schema 与实际执行实现解耦 |
| `RuntimeCallableRegistry + CallDispatcher` | 架构变更 | 统一路由 Tool 与 Skill 激活，调用发现只接受结构化 Call Block |
...
| `SKILL_ACTIVATION` 路由 | 架构变更 | Skill 激活通过统一 Dispatcher 直接转发 Skill Manager，不借用本地文件读取 |
...
| Managed 路径完全禁止本地资源加载 | 产品约束 | ToB Agent 不操作本地文件，Model/Tool/Skill 均来自独立管理器 |
...
| 模块 | 目标职责 |
|---|---|
| `modules/agent-core` | Agent Loop、消息状态、Run 状态、ModelInvoker/CallDispatcher 端口和规范化 Call Block；不依赖文件系统或 Manager SDK |
| `modules/ai-contract` | 规范化 Message、Model Tool Schema、Call Block 和流事件 DTO；Agent Runtime 与独立 Model Manager 共同依赖 |
| 独立 Model Manager | 从当前 `modules/ai` 迁出 Provider Adapter、认证、协议转换和实际模型执行；不与 Agent Session 同生命周期 |
| `modules/managed-agent-runtime` | Published revision client、RemoteManifestLoader、ManagedAgentInstance、Runtime Revision、CallableRegistry、Registry、Bootstrap、Reconciler、三个 Manager Gateway Client、Session/Run 快照 |
...
| 文件或类型 | 目标修改 |
|---|---|
| `Agent.java` | 新增接收 `ModelInvoker`、`ModelBindingRef`、`CallDispatcher`、`PinnedSessionResourceView` 和 `RunStore` 的 Managed 构造路径；Legacy `Agent(CampusClawAiService)` 暂时保留 |
...
| `AgentState.java` | 将 `List<AgentTool>` 拆为模型可见 `RuntimeCallableRegistry` 和执行 Dispatcher；增加 `ModelBindingRef`、active Skill/Tool overlay 和 Run 状态 |
| `AgentContext.java` | 暴露 `modelTools()`、`callableRegistry()`、`executionContext()`，不再用 `AgentTool` 同时表达 Schema 与实现 |
| `AgentLoop.java` | `invokeModel()` 调 `ModelInvoker`；只消费规范化 `CallBlock`，`runToolPhase()` 改为 `runCallPhase()`；返回 `AgentRunResult` 并支持 `Suspended` |
| `ToolExecutionPipeline.java` | 仅保留 Legacy 本地 Tool 路径；Managed 路径不进入该类 |
| `ContextFileLoader.java` | 仅供 Legacy 路径使用；Managed Factory 不构造、不调用，不读取 SYSTEM/APPEND_SYSTEM/AGENTS/CLAUDE 文件 |
| `SystemPromptBuilder.java` | 增加 `buildManagedStatic()` 和 `appendTrustedSessionContext()`；Tool/Skill 清单接收 Manifest Descriptor，Managed 路径不接收 Path 或 cwd |
| `SkillPromptFormatter.java` | 接收 `RuntimeSkillDescriptor`，去掉必须存在本地 `location` 的假设 |
| `SkillLoader.java` | 仅供 Legacy 路径扫描 `SKILL.md`；Managed 路径由 `RemoteManifestLoader` 获取 Skill Manifest，禁止调用本类 |
| `SkillExpander.java` | Legacy 保留；Managed `/skill:name` 由命令层构造显式 Skill 激活请求并交给 `CallDispatcher`，禁止读取 Path |
| `DefaultToolCatalog.java` | 仅供 Legacy 本地 Tool；Managed 路径从 Tool Manifest 构建 Registry，不调用 Spring/Extension/声明式 Tool Source |
...
| `SessionManager.java` | Legacy JSONL 可保留；Managed 路径通过无 cwd 的 `SessionStore` 持久化消息、`SessionSnapshotV2` 和 `RunCheckpointV1`，支持 `WAITING_APPROVAL` 恢复 |
...
com.campusclaw.managed.runtime.PublishedAgentRevision
com.campusclaw.managed.runtime.PublishedAgentResourceBundle
com.campusclaw.managed.runtime.RemoteManifestLoader
...
com.campusclaw.agent.call.NormalizedCall
com.campusclaw.agent.call.CallTargetKind
com.campusclaw.agent.call.RuntimeCallableDescriptor
com.campusclaw.agent.call.RuntimeCallableRegistry
com.campusclaw.agent.call.CallDispatcher
com.campusclaw.agent.skill.RuntimeSkillSet
com.campusclaw.agent.session.PinnedSessionResourceView
...
- 定义 Control Plane 的 `PublishedAgentRevision` 原子发布与按 `publication_id` 精确读取契约。
- 定义 `ModelManager.publishManifest()`、`getManifest()`、`stream()`。
- 定义 Model Manifest、Binding、能力 Descriptor 和规范化流事件。
- 定义跨 Provider 的 `NormalizedAssistantBlock` 和结构化 `CallBlock`，禁止 Runtime 解析普通文本寻找调用。
- 定义 `ToolManager.publishManifest()`、`getManifest()`、`invoke()`。
- 定义 `ToolManager.resume()`、Tool Manifest、Descriptor、InvocationResult 和审批记录约束。
- 定义 `SkillManager.publishManifest()`、`getManifest()`、`activate()`。
...
- 定义所有 Manifest 的 AgentKey、content hash、签名或可信传输校验，以及不可变读取语义。

### 阶段 2：实现 Runtime 装配

- 实现 `compileStructuredSystem()`。
- 实现 `buildManagedStatic()` 和容器级 `static_system_prompt`。
- 实现 `RemoteManifestLoader`，只按 `PublishedAgentRevision` 中的精确 Manifest ID 获取远程资源。
- 实现 `RuntimeCallableRegistry` 和 `PinnedSessionResourceView`。
...
- 实现 Model Manager Gateway 和本地确定性 `ModelSelectionPolicy`。
- 实现 Runtime 和 Config 的 canonical hash。

### 阶段 3：适配 pi-mono-java

- 将模型可见调用改为独立 `RuntimeCallableRegistry`，其中包含 Tool 与 Runtime 保留的 `activate_skill`。
- 将当前完整 `Model` 从 Managed AgentState 中替换为 `ModelBindingRef`，并注入 `ModelManagerInvoker`。
- 实现统一 `CallDispatcher`，按 `CallTargetKind` 转发 Tool Manager 或 Skill Manager。
- 改造 `AgentContext`、`AgentLoop` 和调用结果事件链，只把 Model Manager 的结构化 Call Block 转发到 Dispatcher。
...
- 为 `SystemPromptBuilder` 增加 managed 模式，由固定 Descriptor 渲染 Tool 清单和 Skill 清单；接口不得接收 cwd 或 Path。
- 将 Skill prompt 类型从本地 `Skill(Path)` 解耦，Managed 路径完全绕开 `SkillLoader` 与 `SkillExpander` 文件读取。
- 实现 Runtime 保留的 `activate_skill` 和 Managed `/skill:name` 显式分发；Legacy SkillExpander 只保留在旧路径。
- Managed Factory 不构造 `ContextFileLoader`、`DefaultToolCatalog` 或任何本地资源加载器。
...
- WebSocket 新建/恢复 Session 不调用 Control Plane、`RemoteManifestLoader` 或任何 Manager Manifest API 的边界测试。
- Managed 启动和 Session 路径不得调用 `ContextFileLoader`、`SkillLoader`、`SkillExpander`、`DefaultToolCatalog` 或文件 API 的边界测试。
...
- Anthropic/OpenAI 等 Provider 的结构化调用都归一化为 Call Block；普通文本、ToolResult 和 Skill instructions 中的伪调用不执行的测试。
...
- CallDispatcher 名称白名单、TargetKind 路由、ToolManager.invoke 转发和结果转换测试。
- RuntimeCallableRegistry 名称唯一性、TargetKind 路由和 `activate_skill` 冲突测试。
...
- Tool A 结果只能经下一模型 Turn 触发 Tool B、Skill A 只能经显式 `activate_skill` 触发 Skill B 的测试。
- Skill 父子依赖、固定版本、循环和最大激活深度测试。
- 多 Session 状态隔离测试。
...
1. `CallDispatcher` 和 Manager Gateway 是采用同步、`CompletionStage` 还是 Reactor 接口；无论哪种形式，`ApprovalRequired` 都必须返回并暂停 Run，不能阻塞等待用户。
2. Model Manager 的规范化流协议是复用当前 `AssistantMessageEventStream`，还是定义独立传输 DTO 后由 Client Adapter 转换。
3. `activate_skill` 是否同时暴露模型调用和用户 `/skill:name`；本文默认两者共享同一个 `CallDispatcher`。
4. Runtime Revision 和三类 Manager Manifest 在最后一个 Session Lease/Snapshot 引用释放后还需要保留多长时间。
5. Provider 不支持某个 JSON Schema 关键字时，是否允许经显式配置的有损降级。
6. `NATIVE_ONLY` 兼容模式是 Provider 级、Model 级还是 Agent 级配置。
7. WebSocket 新会话的 AgentKey 是继续使用握手查询参数，还是改为升级后的首个必须命令；本文默认采用握手参数。
8. WebSocket 身份认证与 tenant/user 上下文是在 Reactor Netty 升级前完成，还是由外部 Gateway 注入；本文不将查询参数中的身份信息作为可信值。
9. 管理面将更新标记为 `APPLIED` 时需要全部容器副本、可配置法定数量还是流量加权副本完成回执。
10. 本文默认 Session 采用 `PINNED`；如果后续启用 `LIVE_AT_TURN_BOUNDARY`，需要另行定义消息兼容性和用户告知策略。

以下事项已经固定，不再作为待确认项：Managed 路径不加载任何本地 Agent/Tool/Skill/Prompt/Extension 文件；本地 ToolCatalog 和 SkillLoader 只属于 Legacy 路径；容器按 `PublishedAgentRevision` 的精确 Manifest ID 装配；普通文本不会触发 Tool 或 Skill。

## 20. 版本历史

| 版本 | 日期 | 变更 |
|---|---|---|
| 0.9.0 | 2026-07-22 | 收敛为纯 ToB Managed 路径：Control Plane 原子发布 AgentConfig 与三个精确 Manifest ID，容器通过 RemoteManifestLoader eager 装配且 Session 只持有 PinnedSessionResourceView；禁止本地文件、ToolCatalog、SkillLoader 和 Extension 回退；新增 RuntimeCallableRegistry、统一 CallDispatcher、Provider 结构化 Call 归一化，以及 Tool 调 Tool、Skill 调 Skill 的显式调用规则 |
