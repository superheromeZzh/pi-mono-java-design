...
#### 9.1.1 具体例子：订单查询和退款 Tool

假设本地 Runtime 发现两个 Tool：

```text
get_order
    来源：Extension 或 Spring ToolSource
    用途：根据订单编号查询订单

create_refund
    来源：Extension、SDK Custom Tool 或 Spring ToolSource
    用途：为订单创建退款申请
```

发现和筛选完成后，旧架构放入 Agent 的不是下面这样的纯名称清单：

```text
["get_order", "create_refund"]
```

而是两个已经绑定本地执行实现的 `AgentTool`。以下是概念伪代码，不对应特定 TypeScript 或 Java 语法：

```text
AgentTool getOrderTool:
    name = "get_order"
    description = "根据订单编号查询订单"
    parameters = {
        type: "object",
    ...
    execute(arguments):
        return LocalOrderService.getOrder(
            arguments.order_id
        )

AgentTool createRefundTool:
    name = "create_refund"
    description = "为订单创建退款申请"
    parameters = {
        type: "object",
    ...
    execute(arguments):
        return LocalRefundService.createRefund(
            arguments.order_id,
            arguments.amount,
            arguments.reason
        )
...
Agent 可以直接用 name 找到发现阶段保存的同一个本地对象：

```text
tool = Agent.state.tools.find(
    item.name == "create_refund"
)

result = tool.execute(arguments)
```

这正是旧架构的关键特征：Tool 发现阶段已经同时完成模型 Schema 装配和本地执行实现绑定。ToB 目标设计会拆开这两部分：Agent 只保存 Schema，实际执行交给 Tool Manager。
