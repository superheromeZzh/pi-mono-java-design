...
6. system prompt 包含结构化 `system` 和 Skill 目录；Tool 的 `name`、`description`、完整 `input_schema` 默认只通过模型请求的独立 `tools` 字段发送。
7. 容器启动后可以注册一个长期存活的 `ResolvedAgentRuntime`；每个对话仍创建独立的、有消息状态的 `AgentSession + Agent`。
8. 目标执行路径是 `ToolCallDispatcher` 把模型 tool call 直接转发给 Tool Manager；`ManagerBackedAgentTool` 只是不改造现有 `AgentLoop` 时的过渡适配。
...
2. 基于 Tool 名称生成的条件规则。
3. Tool 的 name 和 description 摘要；这是当前实现的重复提示，不是目标架构必需。
...
目标设计不默认把 Tool 目录拼入 system prompt。`name`、`description` 和完整 `input_schema` 共同进入模型请求的 tools 字段：
...
只有在某个 Provider 不支持原生 tool calling，或产品需要额外的 Tool 选择规则时，才将简短的非权威目录写入 system prompt。不得将完整 schema 在 prompt 和 tools 中维护两份。
...
**架构变更**：Agent Runtime 持有模型可见的 `RuntimeToolCatalog` 和统一 `ToolCallDispatcher`，不再要求每个 Tool 实现 `AgentTool.execute()`。

```text
record RuntimeToolCatalog:
    manifest_id: string
...
function invokeModel(agentContext):
    llmContext = Context(
        ...
        agentContext.runtime_tool_catalog.asModelTools()
    )
...
class ToolManagerToolCallDispatcher
    ...
    dispatch(toolCall, context):
        descriptor = runtime_tool_catalog
        ...
        managerResult = tool_manager.invoke({
            manifest_id:
                runtime_tool_catalog.manifest_id,
...
Agent Runtime 只根据已固定 Catalog 确认 tool name 是否属于当前 Agent，不解析 permission，也不执行 Tool。Tool Manager 使用固定 Manifest 完成 schema 校验、权限审批、实际执行和结果标准化。
...
1. 推荐：在 `RuntimeToolCatalog` 中注册一个 Manager-backed `load_skill` Tool，模型和 `/skill:name` 都通过 `ToolCallDispatcher` 激活 Skill。
...
该方法取代硬编码 `BASE_PROMPT`，但继续复用当前 Skill 格式、项目上下文和环境信息的装配能力。Tool 描述由 `RuntimeToolCatalog` 通过 `Context.tools` 传递，不作为 managed system prompt 的默认组成部分。

## 12. 基于 Runtime 创建 pi-mono-java Session

![基于 ResolvedAgentRuntime 创建 pi-mono-java Session](pi_session_creation.svg)

[PlantUML 源码](diagram.puml#L172)
...
function createManagedSession(
    ...
    runtimeToolCatalog = RuntimeToolCatalog.create({
    ...
    session = AgentSessionFactory.createManaged({
        ...
        model_tools:
            runtimeToolCatalog.asModelTools(),
...
上述 `model_tools + tool_call_dispatcher` 是目标新接口。在过渡模式中，Session Factory 可把同一 Catalog 物化为 `List<ManagerBackedAgentTool>` 传入现有 `agent.setTools()`，但不应把这个兼容形态写入 Manager 契约。
...
| `RuntimeToolCatalog + ToolCallDispatcher` | 架构变更 | 模型工具描述与实际执行解耦 |
...
- 将模型可见 Tool 改为独立 `RuntimeToolCatalog`。
