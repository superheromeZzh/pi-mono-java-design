...
| 基线日期 | 2026-07-22 |
| 设计范围 | Agent 配置编译、Model/Tool/Skill Manager 绑定与执行、pi-mono-java Server WebSocket 中的 Runtime 装配、多 Session、Run、审批与恢复 |
...
![基于 ResolvedAgentRuntime 建立 WebSocket Session 并执行 Run](pi_session_creation.svg)

[PlantUML 源码](diagram.puml#L241)
...
- 新会话必须给出 `agent_id`；省略 `agent_version` 时，只能使用 Deployment Plan 已固定的 active version。
- 恢复会话以 `SessionSnapshotV2` 中的 AgentKey 为权威值。握手同时给出 Agent 身份时必须一致，否则拒绝升级。
...
| `SessionPool.java` | Managed key 改为 `conversation_id + agent_id + agent_version`，通过 Registry 和 Managed Factory 创建；恢复以 Snapshot AgentKey 为准；禁止调用 Tool/Skill refresh |
