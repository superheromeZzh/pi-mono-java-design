...
8. 容器必须在 readiness 之前为部署范围内的每个 Agent 版本完成 `ResolvedAgentRuntime` 装配和注册；Session 创建路径禁止懒加载 Runtime。
9. 目标执行路径是 `ToolCallDispatcher` 把模型 tool call 直接转发给 Tool Manager；`ManagerBackedAgentTool` 只是不改造现有 `AgentLoop` 时的过渡适配。
10. 每个对话仍在打开时创建独立的、有消息状态的 `AgentSession + Agent`；容器启动时不预建空对话。
...
`ResolvedAgentRuntime` 在容器 readiness 之前完成创建。`static_system_prompt` 已包含结构化 system、Tool 清单和 Skill 清单；`runtime_tool_set` 已建立按模型 Tool 名称查找的不可变索引。Runtime 不保存 Tool 或 Skill 的业务实现，也不保存 Manager 内部 permission 结构。
...
当前 `AgentSession.resolveModel(String)` 支持一个字符串和模糊匹配。目标 `model[]` 必须在容器启动的 Runtime 装配阶段由 Model Manager 解析为固定 Model Manifest。Session 创建时可以根据凭据和会话上下文从该固定 Manifest 中选择具体 `Model`，但不得重新查询模型配置或扩展候选集。选定后直接调用 `agent.setModel(model)`。
...
`createManagedSession()` 不调用模型，也不读取 AgentConfig、不调用 `bindAgentTools()`、不调用 `bindAgentSkills()`、不重新编译静态 system prompt。后续 `session.prompt()` 才进入支持 `model_tools + tool_call_dispatcher` 的 AgentLoop。
...
- Session 创建不调用 Config Repository、`bindAgentTools()` 或 `bindAgentSkills()` 的边界测试。
