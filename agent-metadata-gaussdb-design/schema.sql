...
CREATE TABLE agent_metadata (
    ...
    CONSTRAINT pk_agent_metadata PRIMARY KEY (id),
    ...
    CONSTRAINT ck_agent_metadata_use_cases CHECK (
...
COMMENT ON COLUMN agent_metadata.role IS 'Agent identity, expertise, and responsibility scope';
COMMENT ON COLUMN agent_metadata.objective IS 'Long-term objective and success criteria';
COMMENT ON COLUMN agent_metadata.instructions IS 'General work principles, priorities, and behavior requirements';
COMMENT ON COLUMN agent_metadata.tool_policy IS 'Rules for tool selection, result verification, and direct answers';
COMMENT ON COLUMN agent_metadata.safety IS 'Untrusted-content handling, permission boundaries, and confirmation rules';
COMMENT ON COLUMN agent_metadata.completion IS 'Completion conditions, self-checks, and failure reporting';
COMMENT ON COLUMN agent_metadata.response_style IS 'Default language, length, format, and reporting style';
COMMENT ON COLUMN agent_metadata.example IS 'Optional example for highly consistent behavior or output';
COMMENT ON COLUMN agent_metadata.use_cases IS 'JSON array of intent-recognition use case strings';
...
COMMENT ON COLUMN agent_model.agent_id IS 'Logical reference to agent_metadata.id';
...
CREATE TABLE agent_tool_binding (
    ...
    CONSTRAINT ck_agent_tool_binding_permission CHECK (
        permission IS NULL OR permission IN ('deny', 'ask', 'allow')
    )
);

COMMENT ON TABLE agent_tool_binding IS 'Tool binding and optional Agent-level permission override';
COMMENT ON COLUMN agent_tool_binding.agent_id IS 'Logical reference to agent_metadata.id';
...
COMMENT ON COLUMN agent_tool_binding.permission IS 'Agent-level deny, ask, or allow; NULL means inherit the Tool permission';
...
COMMENT ON TABLE agent_skill_binding IS 'Skill references bound to an Agent';
COMMENT ON COLUMN agent_skill_binding.agent_id IS 'Logical reference to agent_metadata.id';
...
CREATE INDEX idx_agent_metadata_updated_at
    ON agent_metadata (updated_at DESC, id);
...
CREATE INDEX idx_agent_tool_binding_permission
    ON agent_tool_binding (permission, agent_id);
...
--     FOREIGN KEY (agent_id) REFERENCES agent_metadata (id)
--     ON DELETE CASCADE;
