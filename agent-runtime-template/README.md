...
| 规范性优先级 | 在 Template、revision/current、Session pinning、相关恢复/GC、Managed Prompt Template 暴露和动态权威发生冲突时，以本文为关联设计的规范性增补 |
| 运行形态 | 单 JVM、多 Agent、多 Session；同 Session 单主 run |

## 1. 结论

`AgentRuntimeTemplate` 应定义为：
...
Session 持久化和业务 pinning key 固定为：

```text
(agent_id, bundle_revision)
```

Registry 的进程内 materialization cache key 为：
...
| 目标设计 | 新增不可变 `AgentRuntimeTemplate`、Template Registry、revision 目录和 Session pinning |
| 架构改造 | 从“每 Session 扫描并组装静态资源”改为“每 revision 加载一次、每 Session 绑定一次” |
| 安全加固 | 模板不保存凭据和权限结果；Model/Tool 动态状态在运行时重新校验 |
| 产品约束 | 同一 Session 同时最多一个主 run；不同 Session 可以并发 |
...
当前活跃设计仍有以下待同步项；本文给出规范性目标，但在这些文件完成反向链接和
协议修订前，不能宣称 Manager-driven Runtime 已完成 Template v1 一致性：

| 待同步文件/符号 | 当前冲突 | 本文规范性结论 |
|---|---|---|
| `pi-mono-java-manager-driven-multi-agent-runtime/README.md` 的 `<agent-id>` 目录替换、cwd/header 恢复 | 可变目录会使旧 Session 的渐进 read 漂移 | 改为不可变 `revisions/<revision>`、原子 current activation record 和 exact header pinning |
| `pi-mono-java-manager-driven-multi-agent-runtime/chat-ws-v2.asyncapi.yaml` 的 `prompt_templates.list` method/schema | Template v1 没有受控 Prompt Template 元数据，却仍可能向 Managed Client 宣告能力 | Managed welcome/capability 必须省略该 method；待 Prompt Template 纳入 bundle manifest 后才能恢复 |
| `pi-mono-java-manager-driven-multi-agent-runtime/chat-ws-v2-client-integration.md` 的 Prompt Template 调用 | Client 可能依据过期 methods 发起调用 | Client 只按服务端协商 methods 启用；Template v1 不展示入口 |

上述是已知集成债，不改变本文的目标合约。关联 README 已增加反向规范链接；
AsyncAPI 和 Client guide 仍必须在实现落地前作为同一协议变更一起修订和校验。
...
| 对象 | 数量与生命周期 | 可变性 | 主要内容 |
|---|---|---|---|
| `AgentRuntimeTemplate` | 每 `(agent_id, bundle_revision, runner_abi)` loaded materialization 一份，可被多个固定同 revision 的 Session 引用 | 深度不可变 | Prompt、Skill 摘要、资源索引、三个通用 Tool spec、来源与兼容信息 |
| `AgentRunner` | 每 JVM 少量常驻实例 | 无会话状态 | 模型调用、Tool 分发、Context 转换等执行算法和共享 Manager client |
| `AgentSession` | 每 `session_id` 一份 | 可变、单写者 | template lease、模型选择、thinking、messages、持久化游标、mailbox、active run 引用 |
| `AgentRun` | 每 `run_id` 一份 | 可变、仅属于一个 Session | trace、deadline、cancel、partial output、pending tools、usage、run sequence |
...
| messages、branch、compaction、JSONL writer/cursor | Session | 对话状态必须隔离且单写 |
...
### 5.6 Prompt Template 与运行策略

当前 bundle 输入只有 Agent、Skill 和 Tool 元数据，没有 Prompt Template 元数据或
受控制品，因此 Template v1 不加载 Java 本地 `prompts/*.md`。Managed WebSocket
的有效 methods 在该能力未定义前必须省略 `prompt_templates.list`；不能一边声明
Managed Session 不读本地 Prompt Template，一边返回进程 cwd 中偶然发现的模板。

如果产品后续增加 Prompt Template 元数据，应把固定版本摘要、entry path 和文件
hash 纳入 manifest、ResourceSnapshot 和新 bundle revision，再恢复该 method。
...
Session header。Session header 的 schema/ABI/provenance 只能从校验成功的 Template
复制。
...
5. create 先强一致查询 Session Store；已有 Session 直接使用 header exact revision，
   不读取 current；
6. 只有 ABSENT 首次创建才通过 `openCurrent` 原子取得 current pointer 和物理
   BundleLease，Registry 再原子取得 CacheEntry lease；
7. Store 以唯一 CAS 写入 `CREATING` header，并且只在 CREATED 分支同事务增加
   revision 持久引用；
8. Factory 创建独立 Session/Agent，持 owner generation 完成后 CAS 为 `READY` 才
   对外可见；
9. `mode=resume` 先读 header，再 acquire exact revision，绝不解析 current；
...
`SessionLifecycleSlot` 是单 JVM 范围内 create/resume/run/evict/delete 的共同
...
`session_id` 创建两套 Session/JSONL writer。
...
| resident Session | 按 Session、history、writer 和 mailbox weight 限制，空闲项按策略淘汰 |
| active run | global + per-agent + Manager/provider admission；同 Session 仍为 0/1 |
| Session mailbox / Store queue | 固定容量；满时拒绝或按协议返回 overload，不无界排队 |
...
- revision 仍被任一可恢复 Session header 引用时不能删除；
- Session Store 必须事务性维护可重建的
  `(agent_id, bundle_revision) -> session_count` 持久引用索引；revision 不要求跨
  Agent 全局唯一，创建 header 后增加引用，业务删除进入不可恢复终态后再减少；
...
| 请求校验 | 仅 Store 判定 ABSENT 的首次创建通过 Repository 原子读取 current activation record 并 pin revision | 捕获漏事件和节点重启期间的发布；已有 Session 始终按 header exact revision |
...
Session 若找不到 header 中的旧 revision，应返回
...
新建 Session 对外返回成功前，持久化 header 至少包含：
...
Session metadata 必须由支持唯一约束、CAS generation 和原子 revision 引用计数的
`SessionMetadataStore` 管理；消息正文仍可继续使用 JSONL。若只使用文件，实现
必须提供进程/文件锁、exclusive create、fsync 和可恢复 WAL，不能依赖当前无事务
的 BufferedWriter 完成这些原子语义。
...
-> if ABSENT:
   ...
   transaction createIfAbsent(candidate CREATING header)
...
Store lookup 和最终 `createIfAbsent` 都必须强一致。lookup 让已持久化 Session 即使在
Agent WITHDRAWN、current 缺失/损坏或新 current 不兼容时，仍能按 header 恢复；它
不能替代最终唯一约束，因为 ABSENT 后仍可能发生竞争。`createIfAbsent` 的返回分支
是规范性协议：

| 结果 | 必须执行的动作 |
|---|---|
| `CREATED(header, ownerGeneration)` | 保留 ABSENT 分支候选 current 的 TemplateLease；该事务唯一增加 `(agent_id, revision)` 持久引用 |
| `EXISTING_CREATING(header)` 且 fingerprint 相同 | 关闭竞争失败后可能存在的候选 current lease（只有 exact ref 相同才可安全复用），按 header acquire 原 revision；CAS `CREATING(N) -> CREATING(N + 1)` 取得新 owner generation，旧 owner 被 fencing；使用 header 中已固定的 model/options 恢复构造 |
| `EXISTING_READY(header)` 且 fingerprint 相同 | 关闭可能存在的候选 current lease；返回同一 resident Session，或进入按 header exact revision 的 resume 流程；不得再次增加 revision 引用 |
...
WITHDRAWN，重试也直接按持久 header 恢复 A，不依赖 B/current。只有 lookup
观察到 ABSENT 的首次创建才读取 current；若最终唯一写输给并发创建，再丢弃候选
并使用胜者 header。Factory 和 READY CAS 都必须携带 owner generation；迟到的旧
...
CREATING 并减少 revision 引用。不得释放 ID 后让第二个 writer 重新创建，也不得
在进程崩溃后复用该 ID。
...
-> load READY Session header and generation
...
仍在内存中的 Session 复用原 lease，不重新读取 current。
...
本文范围是单 JVM；所有 create/resume/run/evict/delete 必须经过同一个
...
并转清理，要么先完整发布后再由 delete 清理，不会在 tombstone 之后复活。未来
扩展多 JVM owner 时，必须把该 slot 替换为带 lease/epoch 的分布式 fencing 协议，
不能沿用单 JVM 假设。
...
不能越过仍在写 Store 的 run 直接关闭 writer。DELETING 的 cleanup owner generation
是唯一仍可写 terminal run/flush 记录的 writer，其他旧 generation 全部被 fencing。

唯一 cleanup owner 在 map 中保留 EVICTING/DELETING slot，使后续 resume/delete
只能 join Future；随后依次完成 run drain、flush、关闭 writer/TemplateLease、释放
resident permit。删除路径最后事务 CAS `DELETING -> DELETED` 并 exactly-once
...
| 当前位置 | 目标改造 | 分类 |
|---|---|---|
| Runtime bundle compiler / AgentDirectoryResolver | 生成不可变 revision 目录和 current activation record；增加 exact revision 解析 | 架构改造 |
| `AgentSession.initialize()` | Managed 路径接收 TemplateLease、ModelDescriptor 和恢复状态，不再扫描/构建静态资源 | 架构改造 |
| `ManagedAgentSessionFactory` | acquire/pin Template；创建 per-session Tool binding 和独立 Agent/Session | 架构改造 |
| `SessionPool.getOrCreate()` | 改为 `session_id -> SessionLifecycleSlot`，统一 create/resume/run/evict/delete；完成持久化且持有 owner generation 后才发布 ready | 并发修复 |
| Session header/Store | 持久化 agent/version/revision/schema/ABI/model；resume 精确加载旧 revision | 架构改造 |
...
| Legacy CLI | 继续使用本地 cwd/Settings/ResourceLoader 路径 | 兼容要求 |
...
- 同一 `session_id` 并发 create 只创建一个 Session/JSONL writer；
...
- 进程在 revision A 的 CREATING 提交后崩溃、current 切到 B 时，同 fingerprint
  重试关闭候选 B、按 header 恢复 A，且不重复增加 revision 引用；
- A 的已持久 Session 在 Agent WITHDRAWN、current 缺失/损坏或不兼容时仍先命中
  Store，并按 header exact A 恢复；不存在的 Session 才依赖 current；
...
- eviction 与 delete 并发时只安装一个 cleanup owner，delete 只升级 terminal intent，
  writer/lease/permit/refcount 均 exactly-once 清理；
