...
> 文档性质：官方文档索引与创建形态梳理
>
> 仓库基线：`8b031af44edb42abf174a03c36259d326313ebb1`
...
本版对全文链接进行了内容级复核，不以“域名官方、页面可打开”作为通过标准。每个入口同时检查官方归属、当前状态、页面语义、资源层级和是否存在更直接的创建文档。当平台公开了对应对象的独立资源 API 时，具体的 create API Reference 必须作为主入口，操作指南只能作为补充；只有代码定义、控制台创建或文件装载路径没有独立资源 API 时，指南才可作为主入口。详表使用以下标记：

| 标记 | 判定标准 |
| --- | --- |
| **主入口** | 独立资源优先采用对象级 create API Reference；非独立资源采用直接包含定义、上传、注册或构建步骤的页面 |
...
| --- | --- | --- | --- | --- |
| OpenAI | [Agents SDK Quickstart](https://developers.openai.com/api/docs/guides/agents/quickstart)、[Agent definitions](https://developers.openai.com/api/docs/guides/agents/define-agents) | [Function calling](https://developers.openai.com/api/docs/guides/function-calling) | [Create a Skill API](https://developers.openai.com/api/reference/resources/skills/methods/create)、[Codex Build skills](https://learn.chatgpt.com/docs/build-skills) | Agent/Tool 代码定义；API Skill 为独立版本化资源；Codex Skill 为文件目录 |
...
| Microsoft Foundry | [Create an Agent REST API](https://learn.microsoft.com/en-us/rest/api/aifoundry/project/agents) | [Build and register MCP server](https://learn.microsoft.com/en-us/azure/foundry/mcp/build-your-own-mcp-server) | [Use skills with Responses API](https://learn.microsoft.com/en-us/azure/foundry/openai/how-to/skills)、[Agent Framework Skills](https://learn.microsoft.com/en-us/agent-framework/agents/skills) | Agent 为独立版本化资源；Tool 可配置/注册；Skill 可上传或代码定义 |
| Google Agent Platform / ADK | [ADK Agents](https://adk.dev/agents/)、[reasoningEngines.create API](https://docs.cloud.google.com/gemini-enterprise-agent-platform/reference/rest/v1beta1/projects.locations.reasoningEngines/create) | [Custom Tools](https://adk.dev/tools-custom/) | [Skills for ADK agents](https://adk.dev/skills/) | Agent、Tool、Skill 可代码定义；云端 Agent Engine 为独立资源 |
| Amazon Bedrock AgentCore | [CreateAgentRuntime API](https://docs.aws.amazon.com/bedrock-agentcore-control/latest/APIReference/API_CreateAgentRuntime.html) | [Harness Tools](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/harness-tools.html)、[CreateCodeInterpreter API](https://docs.aws.amazon.com/bedrock-agentcore-control/latest/APIReference/API_CreateCodeInterpreter.html) | [Agent Registry quickstart](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/registry-get-started.html) | Agent Runtime 和部分 Tool 为独立资源；Skill 可登记为注册表资源 |
...
- **主入口**：[Create a Skill API](https://developers.openai.com/api/reference/resources/skills/methods/create)：`POST /v1/skills` 上传目录文件或单个 ZIP，创建独立 Skill 资源并返回默认版本和最新版本信息。
- **补充**：[Skills](https://developers.openai.com/api/docs/guides/tools-skills)：说明 Skill 包结构、版本创建、默认版本更新、删除约束，以及在 hosted shell 和 local shell 中的装载方式。
- **主入口**：[Codex Build skills](https://learn.chatgpt.com/docs/build-skills)：Codex 的 `SKILL.md` 目录结构、创建和复用方式。

这两类 Skill 均采用可复用指令与资源包的思路，但生命周期不同：OpenAI API Skill 具有 create、retrieve、update、delete、list 和版本管理接口；Codex Skill 是由 Codex 发现和装载的文件目录，不应以同一 CRUD 资源理解。
...
- **主入口**：[Create an Agent REST API](https://learn.microsoft.com/en-us/rest/api/aifoundry/project/agents)：`POST /agents` 创建 Agent 或既有 Agent 的新版本，定义可为 hosted、prompt、workflow 或 external。
- **补充**：[Prompt agent quickstart](https://learn.microsoft.com/en-us/azure/foundry/agents/quickstarts/prompt-agent)：通过 portal、SDK 或 REST 完成 Prompt Agent 的创建和调用流程。
...
- **主入口**：[reasoningEngines.create API](https://docs.cloud.google.com/gemini-enterprise-agent-platform/reference/rest/v1beta1/projects.locations.reasoningEngines/create)：`POST /v1beta1/{parent}/reasoningEngines` 创建云端 Reasoning Engine 资源。
- **补充**：[Agent Runtime ADK quickstart](https://docs.cloud.google.com/gemini-enterprise-agent-platform/build/runtime/quickstart-adk)：覆盖 ADK Agent 开发、云资源创建、部署和测试的完整流程。
...
- **主入口**：[CreateAgentRuntime API](https://docs.aws.amazon.com/bedrock-agentcore-control/latest/APIReference/API_CreateAgentRuntime.html)：通过 AgentCore Control Plane 创建独立 Agent Runtime 资源。
- **补充**：[AgentCore quickstart](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/agentcore-get-started-cli.html)：使用 `agentcore create` 创建 Harness 或代码型 Agent 项目并部署。
...
- **主入口**：[CreateCodeInterpreter API](https://docs.aws.amazon.com/bedrock-agentcore-control/latest/APIReference/API_CreateCodeInterpreter.html)：创建独立的托管 Code Interpreter 资源。
- **补充**：[Creating a Code Interpreter](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/code-interpreter-create.html)：通过控制台、CLI 或 SDK 创建托管 Tool 的完整操作流程。
...
1. 托管资源 API：Anthropic、Microsoft Foundry、Google Agent Platform、AWS AgentCore、阿里云、腾讯云、Coze OpenAPI、LangSmith Assistant；
2. SDK/代码定义：OpenAI Agents SDK、Google ADK、LangChain；
...
本轮再次逐页检查总览和详表中的所有官方链接，并增加“独立资源必须优先列对象级 create API Reference”的规则。当前修正项与上一轮保留的审查记录如下：

| 平台 | 原入口或原判断 | 审查结果与处理 |
| --- | --- | --- |
| OpenAI | Skills 指南被列为 API Skill 创建主入口 | 改为对象级 [Create a Skill API](https://developers.openai.com/api/reference/resources/skills/methods/create)；Skills 指南降为包结构、版本和装载流程补充；Codex 文件型 Skill 继续单列 |
| Microsoft | Prompt agent quickstart 被列为唯一 Agent 主入口 | 增加对象级 [Create an Agent REST API](https://learn.microsoft.com/en-us/rest/api/aifoundry/project/agents) 为主入口，quickstart 改为端到端补充 |
...
| Google | 上一轮以完整流程为由用 quickstart 取代对象级 API Reference | 恢复 [reasoningEngines.create API](https://docs.cloud.google.com/gemini-enterprise-agent-platform/reference/rest/v1beta1/projects.locations.reasoningEngines/create) 为云资源创建主入口，Agent Runtime ADK quickstart 保留为补充，两者分别回答“接口是什么”和“如何完成部署” |
| AWS | AgentCore quickstart 和 Code Interpreter 指南被列为资源创建主入口 | 增加 [CreateAgentRuntime API](https://docs.aws.amazon.com/bedrock-agentcore-control/latest/APIReference/API_CreateAgentRuntime.html) 与 [CreateCodeInterpreter API](https://docs.aws.amazon.com/bedrock-agentcore-control/latest/APIReference/API_CreateCodeInterpreter.html) 为对象级主入口，原指南保留为操作流程补充 |
...
其余链接经内容级检查后保留。国产平台中，阿里云、腾讯云已有对象级创建 API，继续以 API Reference 为主；Coze、Dify、百度千帆没有用运行时 API 或泛化概述冒充未公开的同层级创建 API。只覆盖运行时、旧版产品或相邻概念的页面统一标为“边界”，不与创建主入口混列。

### 7.2 已观察事实

- 表格和详表中的创建方式均来自 2026-07-21 可访问的官方资料。
- OpenAI API Reference 已公开 Skills 的 CRUD、列表和版本接口，因此不能再把它描述为只有指南级上传链路。
...
| 版本 | 日期 | 变更 |
| --- | --- | --- |
| 2.2.0 | 2026-07-21 | 按“对象级 create API Reference 优先”规则重新全量审查；OpenAI Skill 改用 Create API；补充 Microsoft Foundry Create Agent、Google reasoningEngines.create、AWS CreateAgentRuntime 与 CreateCodeInterpreter API；保留对应指南为补充 |
