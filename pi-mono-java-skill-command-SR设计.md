...
本 SR 的 9 个设计图统一由 [`diagrams/skill-command/diagram.puml`](./diagrams/skill-command/diagram.puml) 生成，其中包括原有的端到端调用时序图。每张图下方的源码链接直接定位到对应命名的 `@startuml`；Markdown 只引用生成后的 SVG，不复制维护第二份图形定义。
...
| Prompt 顺序 | Template 后 Skill | Skill 后 Template | 移除 Prompt Template 输入展开，Skill 处理后直接进入 Agent Prompt |
...
PlantUML：[查看源码](./diagrams/skill-command/diagram.puml#L156)
...
PlantUML：[查看源码](./diagrams/skill-command/diagram.puml#L187)
...
![Skill Command 端到端调用时序](./diagrams/skill-command/08-end-to-end-invocation.svg)

PlantUML：[查看源码](./diagrams/skill-command/diagram.puml#L217)
...
private String expandUserInput(String text) {
    return skillCommandExpander.expand(
            text,
...
已注册 Command 在进入该辅助方法之前由统一 Dispatcher 处理；Steer 和 Follow-up 不进入注册命令执行路径，保持 pi 对 Extension Command 的拒绝语义。Prompt Template 不再是 `expandUserInput()` 的一部分。
...
PlantUML：[查看源码](./diagrams/skill-command/diagram.puml#L324)
...
- GUI 补全、Help、System Prompt 和 AgentSession 统一读取 `resourceLoader.getSkills()`。
...
- AgentSession 只执行 Skill 展开，移除 Prompt Template 命令展开。
...
- 更新 GUI 补全、Help、Reload 和 Session Skill 查询。
...
| TD-SC-09 | Skill 与 Prompt Command 的路由 | pi 的 Prompt、Steer、Follow-up 都是 Skill-first、Template-second | Java 目标移除 Prompt Command，AgentSession 私有辅助方法只展开 Skill | 保留本地文件 Prompt Template；与 GUI ToB 的数据库持久化方向不符，且扩大文本路由命名空间 | 路由更简单；未来 Prompt Library 需要独立的结构化 GUI 调用设计 |
