...
- 文档版本：1.1.0
- 调研日期：2026-07-17
- 范围：Claude Code、Codex、OpenCode、Cline、Qwen Code、Kiro CLI、Amp、Grok Build、OpenClaw、Goose、Coze Studio、Continue、Devin、Pi，以及 MCP 基线
...
|---|---|---:|---|---:|---|
| Claude Code / Managed Agents | 内置/托管 Tool、MCP、客户端 Custom Tool | 条件式：T2/T4；客户端 Tool 仅 T0 | `.claude/agents/*.md`、`--agents` JSON；Managed Agent 版本化资源 | 是，A1/A2 | Agent 成熟；任意 Tool 仍需 MCP Server 或应用执行 |
...
### 4.1 Claude Code 与 Claude Managed Agents

**观察到的 Tool 行为**

- Claude 的 Tool 定义包含名称、描述和 `input_schema`，但这只是模型与应用之间的调用契约。官方明确区分客户端 Tool 与服务端 Tool：客户端 Tool 由应用执行，服务端 Tool 由 Anthropic 基础设施执行。[How tool use works](https://platform.claude.com/docs/en/agents-and-tools/tool-use/how-tool-use-works)、[Tool reference](https://platform.claude.com/docs/en/agents-and-tools/tool-use/tool-reference)
...
- Agent：达到 A1；Managed Agents 达到 A2。
- Tool：MCP 达到 T2，托管工具达到 T4；用户自定义 Tool 的描述本身只有 T0，应用仍须执行。
...
| 项目 | 基线 | 重点路径或官方资料 |
|---|---|---|
| Codex | `315195492c80fdade38e917c18f9584efd599304` | `codex-rs/protocol/src/dynamic_tools.rs`; `codex-rs/core/src/tools/handlers/dynamic.rs`; `codex-rs/core/src/config/agent_roles.rs` |
| OpenCode | `3a1c6df9e24672f0761a6ced18e1315d89334baf` | `packages/opencode/src/tool/{tool,registry}.ts`; `packages/core/src/config/plugin/agent.ts`; `packages/opencode/src/agent/agent.ts` |
| Cline | `c564045d8135c0c1c330b21d47b68b74917ce614` | `sdk/packages/shared/src/agent.ts`; configured agent config/tool；`agent-runtime.ts` |
| Qwen Code | `0ecba4b3c709d271a17faa5ac9537bf1b102eaf1` | `packages/core/src/tools/tool-registry.ts`; `packages/core/src/subagents/*` |
| Grok Build | `8adf9013a0929e5c7f1d4e849492d2387837a28d` | `xai-grok-agent/src/{config,builder}.rs`; `xai-grok-tools/.../tool_metadata.rs`; `xai-tool-runtime/src/tool.rs` |
| OpenClaw | `55bb6cfa2ec4863b5ece006aee70b5de6d967709` | `src/config/types.agents.ts`; `src/plugins/*`; `src/agents/tools/common.ts` |
| Goose | `d6345f75dba4f43afe705e045414edf173e40af4` | `crates/goose/src/recipe/*`; `crates/goose/src/agents/{extension,extension_manager,agent}.rs` |
| Coze Studio | `22275b1c2661d35344a7493cffe401e8cc61cf8e` | `backend/domain/plugin/*`; `backend/domain/agent/singleagent/*`; crossdomain models |
| Continue | `d0a3c0b626b5bebc3bef4742eec05a0242be0bab` | `packages/config-yaml/src/*`; `core/tools/*`; `extensions/cli/src/subagent` |
| Pi | `216e672e7c9fc65682553394b74e483c0c9e47f7` | `packages/agent/src/types.ts`; `packages/coding-agent/src/core/extensions/*`; README |
| MCP | `26897cc322f356487da89113451bd16b520b9288` | `schema/2025-11-25/schema.ts` |
| Claude Code / Managed Agents | 官方文档，访问 2026-07-17 | Subagents、Tool use、Managed Agent setup/tools/MCP connector |
| Kiro CLI | 官方文档，访问 2026-07-17 | v3 Agent config、MCP configuration |
| Amp | 官方文档，访问 2026-07-17 | Plugin API、Custom Agents |
| Devin | 官方文档，访问 2026-07-17 | Session API、Playbooks、Skills、Devin MCP |
...
以下类图表达的是各项目**观察到的核心类型关系或官方公开配置合约**，并非声称它们共享同一个 `ToolDefinition` 接口。为保证字段可读性，按执行机制分为四组；UI、日志和供应商内部字段只保留与 Tool 注册、调用或执行直接相关的部分。
...
- 这三者都不通过 Agent 配置重新定义 MCP Tool 的执行行为，定义和实现仍归 Server 所有。
...
- `plantuml -tsvg diagram.puml` 生成七个 SVG；
- SVG 均通过 XML 解析；
- Markdown 不含 Mermaid；
