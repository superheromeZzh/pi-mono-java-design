...
> Java GUI 源码基线：[`b99871a0321b73606a8f074c42050f28f52fdfca`](https://github.com/superheromeZzh/pi-mono-java/tree/b99871a0321b73606a8f074c42050f28f52fdfca)
...
- Java 产品只提供 GUI；GUI 的聊天输入、命令面板和按钮统一进入同一目录、分发器和 Handler。
- 注册表示公开：每个可执行命令都会进入目录、帮助和审计。
- 客户端能力通过 Context 端口检测；命令身份与具体客户端解耦。

本设计以 pi Built-in Command 的源码行为为主要基线，但不复制其集中式字符串路由，也不复制 TUI 专属输入语法。Java 版本把 pi 分散在元数据、补全、执行分支和会话控制流中的事实，收敛为一个不可变定义和一条供 GUI 使用的统一执行管线。
...
| Java 产品只提供 GUI | Agent UI 的聊天输入、命令面板和按钮复用同一执行管线 |
| ToB 中所有命令均为公开产品能力 | 注册同时驱动执行、帮助、补全和审计 |
| pi Built-in 是主要行为基线 | 执行模式、路由顺序和补全能力必须能追溯到固定源码 |

“只提供 GUI”是 Java 产品约束，不是 pi 的既有行为。pi 的 TUI 源码用于提取命令语义；Java GUI 源码用于确定输入、展示和传输边界。
...
固定源码基线中的 pi 没有第一方 GUI 或 Web UI。Git 树的 `packages/` 只有 `agent`、`ai`、`coding-agent` 和 `tui`；[`README.md`](https://github.com/badlogic/pi-mono/blob/bb959aae017eedc8edaa91d01d0475d483ea9371/packages/coding-agent/README.md#L20-L24) 将产品定义为 terminal coding harness，并列出 Interactive、Print/JSON、RPC 和 SDK 四种使用方式。

其中：

- Interactive 使用 `ProcessTerminal` 和 `TUI`，是终端界面；
- Print/JSON 是非交互命令行输出；
- [`RPC mode`](https://github.com/badlogic/pi-mono/blob/bb959aae017eedc8edaa91d01d0475d483ea9371/packages/coding-agent/docs/rpc.md#L1-L5) 是基于 stdin/stdout JSONL 的无界面集成协议，可被 IDE、自定义 GUI 或自动化客户端调用，但自身不渲染 GUI；
- SDK 提供嵌入能力，也不是现成 GUI。

历史上的 `packages/web-ui` 已在 pi 提交 [`b141e1fa2460868686ffd19c5d4ced743eee6c24`](https://github.com/badlogic/pi-mono/commit/b141e1fa2460868686ffd19c5d4ced743eee6c24) 删除。工作区里被 `.gitignore` 命中的旧 `dist` 文件不属于当前源码基线，不能作为 pi 现有 GUI 的证据。

### 3.6 Java 当前 GUI 实现与目标差距

Java 工程已经包含 Vue GUI，但命令定义和执行仍在前端分散实现：

- [`slashCommands.ts`](https://github.com/superheromeZzh/pi-mono-java/blob/b99871a0321b73606a8f074c42050f28f52fdfca/frontend/src/chat/slashCommands.ts#L13-L122) 定义前端命令清单、解析和建议排序；
- [`slashCommandExecutor.ts`](https://github.com/superheromeZzh/pi-mono-java/blob/b99871a0321b73606a8f074c42050f28f52fdfca/frontend/src/chat/slashCommandExecutor.ts#L71-L163) 使用 `switch` 执行本地动作或调用 WebSocket action；
- [`App.vue`](https://github.com/superheromeZzh/pi-mono-java/blob/b99871a0321b73606a8f074c42050f28f52fdfca/frontend/src/App.vue#L112-L180) 提供 `/` 建议、解析和普通 Prompt 回退；
- [`ClientCommand`](https://github.com/superheromeZzh/pi-mono-java/blob/b99871a0321b73606a8f074c42050f28f52fdfca/frontend/src/types/ws.ts#L96-L108) 当前没有 Bash 请求。

因此目标设计保留 GUI 交互，但把注册命令目录、执行策略和服务端业务行为收敛到 Java 命令核心。GUI 只负责展示目录、构造 `CommandInvocation`、承载交互和渲染结果；确实只影响本地视图的动作可以继续留在 GUI，但必须明确标注为 GUI Local Action，不冒充服务端注册命令。
...
| 展示层 | `Agent UI` | 接收聊天输入、命令面板或按钮动作，展示交互、输出和刷新结果 |

关键依赖方向为“Agent UI → 命令核心 → Handler → 应用服务”。Handler 不依赖 Vue 组件、WebSocket DTO 或桌面框架类。
...
[查看 PlantUML 源码](./diagrams/command/diagram.puml#L119)
...
| `description` | 是 | 面向用户的命令说明 | 同时供补全、帮助和 GUI Command Palette 使用 |
...
`description` 在 trim 后必须包含内容。定义层不使用正文正则，也不设置统一长度上限；GUI 在展示时根据组件宽度执行单行化和截断。该边界参考 pi 将 Skill description 投影到补全项、再由 [`SelectList`](https://github.com/badlogic/pi-mono/blob/bb959aae017eedc8edaa91d01d0475d483ea9371/packages/tui/src/components/select-list.ts#L98-L99) 处理显示宽度的方式，但渲染责任落在 Java GUI。
...
| GUI 运行环境能力 | Context 端口报告剪贴板、文件选择器、下载和外部浏览器等能力 | Handler 不依赖具体浏览器或桌面容器 API |
...
| Agent UI | 将 Interaction、Output 和刷新提示渲染为用户可见界面 |
...
[查看 PlantUML 源码](./diagrams/command/diagram.puml#L226)
...
[查看 PlantUML 源码](./diagrams/command/diagram.puml#L262)
...
Agent UI 的 Command Palette 和按钮不需要先拼接成文本；它们提交结构化命令请求，由服务端构造 `CommandInvocation`，但仍必须经过同一个 Dispatcher 和 ExecutionGate。聊天输入框使用上述文本路由。

#### 7.1.1 pi Bash 行为与 Java GUI 边界

以下内容是 pi 源码事实，不是 Java GUI 目标能力：`!` 与 `!!` 是 pi Interactive TUI 的输入语法。TUI 输入路由器移除前缀后，把剩余文本交给 Bash 执行器；这类输入不是注册 Command，也不是发送给 Agent 的普通 Prompt。

pi RPC 还提供结构化 `type: "bash"` 请求，并通过 `excludeFromContext` 表达是否进入 LLM 上下文。这只能证明 RPC client 可以请求 Bash，不能证明 pi 自带 GUI。

| pi 入口 | 调用方式 | 已观察行为 |
|---|---|---|
| Interactive TUI | 用户输入 `!command` 或 `!!command` | 是，`interactive-mode.ts` 解析前缀 |
| RPC client | 发送 `{ type: "bash", command, excludeFromContext }` | 是，RPC mode 直接调用 `AgentSession.executeBash()` |

| 输入形式 | 执行行为 | 会话记录 | 后续 LLM 上下文 |
|---|---|---|---|
| `!git status` | 执行 `git status` 并向客户端显示输出 | 记录命令、输出、退出码和取消状态 | 包含该次 Bash 执行结果，后续 Agent 可以使用 |
| `!!git status` | 执行 `git status` 并向客户端显示输出 | 同样写入会话记录，并标记 `excludeFromContext=true` | 组装模型消息时过滤该记录，后续 Agent 看不到 |

因此，`!!` 的“无上下文”只表示不发送给 LLM，不表示不执行、不展示或不保存会话记录。它适合只供开发者查看、输出很大或与后续推理无关的 Shell 操作。

在 pi TUI 中，Bash 路径位于 streaming 和 compaction 路由之前。Agent 正在生成回复时，Bash 命令仍可开始执行，输出显示在 pending 区域；当前这次 Agent 生成不会读取新结果。Agent 结束后，`!` 的结果进入后续 LLM 上下文，`!!` 的结果继续保持排除。系统同一时间只执行一个 Bash 命令，已有 Bash 运行时，新调用提示用户先取消当前命令。

pi 的 TUI 路由判断见 [`setupEditorSubmitHandler()`](https://github.com/badlogic/pi-mono/blob/bb959aae017eedc8edaa91d01d0475d483ea9371/packages/coding-agent/src/modes/interactive/interactive-mode.ts#L2675-L2693)：一个 `!` 设置普通 Bash，两个 `!` 设置 `excludeFromContext`。执行与 pending 展示见 [`handleBashCommand()`](https://github.com/badlogic/pi-mono/blob/bb959aae017eedc8edaa91d01d0475d483ea9371/packages/coding-agent/src/modes/interactive/interactive-mode.ts#L5637-L5724)。pi 将两种结果都记录为 `BashExecutionMessage`，见 [`AgentSession.recordBashResult()`](https://github.com/badlogic/pi-mono/blob/bb959aae017eedc8edaa91d01d0475d483ea9371/packages/coding-agent/src/core/agent-session.ts#L2610-L2635)；[`convertToLlm()`](https://github.com/badlogic/pi-mono/blob/bb959aae017eedc8edaa91d01d0475d483ea9371/packages/coding-agent/src/core/messages.ts#L148-L160) 只过滤带 `excludeFromContext` 的结果。

pi 的非 TUI 入口见 [`RpcCommand`](https://github.com/badlogic/pi-mono/blob/bb959aae017eedc8edaa91d01d0475d483ea9371/packages/coding-agent/src/modes/rpc/rpc-types.ts#L50-L53) 与 [`rpc-mode.ts`](https://github.com/badlogic/pi-mono/blob/bb959aae017eedc8edaa91d01d0475d483ea9371/packages/coding-agent/src/modes/rpc/rpc-mode.ts#L548-L559)。RPC 直接接收 Bash 命令和 `excludeFromContext`，没有解析 `!`/`!!` 文本前缀。

Java GUI 当前 [`ClientCommand`](https://github.com/superheromeZzh/pi-mono-java/blob/b99871a0321b73606a8f074c42050f28f52fdfca/frontend/src/types/ws.ts#L96-L108) 没有 Bash 请求，本设计也不提供 Shell 能力。因此 GUI 不展示 Bash 入口，不解析 `!`/`!!`，这两种输入按普通 Prompt 处理。
...
Java 的统一 Invocation 属于架构改造：Parser 接受连续空白作为名称与参数的分隔符，并只执行一次结构拆分；具体参数语法仍由 Handler 决定。这样 GUI 的聊天输入与 Command Palette 使用相同 Invocation 规则，也避免 pi 中各命令分别切片产生不一致。

## 8. 统一分发

![注册命令高层分发时序](./diagrams/command/07-dispatch-sequence.svg)

[查看 PlantUML 源码](./diagrams/command/diagram.puml#L299)

本图只表达 SR 级产品组件协作：

1. User 在 Agent UI 提交命令；
2. Agent UI 把结构化命令请求发送到 Agent Gateway；
3. Agent Gateway 完成身份和会话上下文绑定，再把请求分发到 Agent Runtime；
4. Agent Runtime 在统一运行时规则下识别、准入并执行注册命令；
5. 只有模型相关命令才调用 Model Manager，其他命令调用各自的领域能力；
6. 命令结果或拒绝结果按 Agent Runtime → Agent Gateway → Agent UI 的路径返回；
7. Agent UI 向 User 展示最终结果。

`CommandParser`、`CommandCatalogSnapshot`、`CommandExecutionGate`、`CommandHandler` 以及取消、超时、锁释放等内部步骤属于 Agent Runtime 的 LLD，不在 SR 顺序图中展开。本 SR 仍在类设计和 Execution Policy 章节定义这些契约，供后续 LLD 实现使用。
...
[查看 PlantUML 源码](./diagrams/command/diagram.puml#L342)
...
### 9.4 `CommandConcurrency`

`CommandConcurrency` 只决定命令与命令能否同时执行。它不决定 Agent 是否必须空闲；命令与 Agent 状态的关系由 `CommandExecutionMode` 决定。

| 枚举值 | 具体含义 | 适用场景 |
|---|---|---|
| `SHARED` | 命令可以与其他 `SHARED` 命令同时执行；遇到 `EXCLUSIVE` 命令时不能同时执行 | 读取会话信息、复制内容、查看帮助等只读或并发安全的操作 |
| `EXCLUSIVE` | 命令执行期间独占命令运行环境，不能与 `SHARED` 或另一个 `EXCLUSIVE` 命令同时执行 | 修改模型、设置、认证状态、会话结构或应用生命周期的操作 |

同一应用实例中的并发关系如下：

| 当前正在执行的命令 | 新 `SHARED` 命令 | 新 `EXCLUSIVE` 命令 |
|---|---|---|
| 无 | 可以立即执行 | 可以立即执行 |
| 一个或多个 `SHARED` | 可以同时执行 | 等待当前命令结束 |
| 一个 `EXCLUSIVE` | 等待当前命令结束 | 等待当前命令结束 |

普通命令发生并发冲突时按到达顺序等待；已有 `EXCLUSIVE` 命令等待时，新的 `SHARED` 命令不能越过它，以免独占命令长期得不到执行。等待执行权的时间不计入命令 `timeout`。

`RUNTIME_TRANSITION` 和 `SHUTDOWN` 具有额外的生命周期规则：运行时切换期间的新调用返回 `COMMAND_BUSY`；关闭命令先停止接收新调用，再在关闭期限内等待或取消正在执行的命令。这些行为由 `mode` 决定，不属于 `SHARED`/`EXCLUSIVE` 本身的含义。

pi 没有 `CommandConcurrency` 类型。pi 的 [`BuiltinSlashCommand`](https://github.com/badlogic/pi-mono/blob/bb959aae017eedc8edaa91d01d0475d483ea9371/packages/coding-agent/src/core/slash-commands.ts#L13-L16) 只有名称和说明；命令主要在单个 TUI [`setupEditorSubmitHandler()`](https://github.com/badlogic/pi-mono/blob/bb959aae017eedc8edaa91d01d0475d483ea9371/packages/coding-agent/src/modes/interactive/interactive-mode.ts#L2541-L2674) 回调中通过局部 `await` 执行，没有共享/独占并发契约。

因此该枚举属于 Java 架构改造：即使只有一个 GUI，Agent streaming、聊天输入、Command Palette、按钮以及异步回调仍可能重叠触发命令，需要由 `CommandExecutionGate` 统一处理并发。它与 `mode` 分开，是因为同为 `IMMEDIATE` 的 `/copy` 和 `/model` 具有不同并发要求：前者可以安全共享执行，后者修改当前模型，需要独占执行。

### 9.5 pi 22 个 Built-in 的条件映射

下表用于说明“如果 Java GUI 采用同名能力，应使用什么策略”，不是要求 Java 完整复制 pi 的 22 个命令。Java 只注册经过产品确认且能在 GUI 中完整交付的命令；GUI Local Action 不因名称相似就进入服务端目录。
...
| `quit` | `SHUTDOWN` | `EXCLUSIVE` | 仅适用于拥有进程生命周期的桌面 GUI；Web GUI 的断开连接或关闭页面属于本地动作，不得关闭服务端 |

`tree` 是有意的 Java 架构差异。pi 在提交路由中可直接打开树，但实际导航会修改 session branch，见 [`AgentSession.navigateTree()`](https://github.com/badlogic/pi-mono/blob/bb959aae017eedc8edaa91d01d0475d483ea9371/packages/coding-agent/src/core/agent-session.ts#L2702-L2795)。Java GUI 在 Agent streaming 时仍可触发导航动作；导航与生成会同时修改会话分支，因此要求 idle。
...
[查看 PlantUML 源码](./diagrams/command/diagram.puml#L375) · [查看 pi `/model` 补全源码](https://github.com/badlogic/pi-mono/blob/bb959aae017eedc8edaa91d01d0475d483ea9371/packages/coding-agent/src/modes/interactive/interactive-mode.ts#L506-L533)
...
[查看 PlantUML 源码](./diagrams/command/diagram.puml#L406)
...
Agent UI 把 `select` 渲染为选择对话框、下拉面板或内嵌列表。Handler 通过同一方法表达“选择模型”意图，Agent UI 负责构造具体组件或交互 DTO。

端口解耦在单一 GUI 中仍然必要：浏览器、桌面 WebView 和原生容器对剪贴板、文件选择器、下载和外部浏览器的支持可能不同。能力不可用时，命令按以下流程返回稳定结果：

1. Context 中的能力端口返回 `UNAVAILABLE`；
2. Handler 返回稳定 `CAPABILITY_UNAVAILABLE`；
3. 客户端显示可操作说明；
4. Handler 不需要知道当前 GUI 使用哪种容器或组件库。

## 12. 新命令开发示例

![New internal command development](./diagrams/command/11-new-command-development.svg)

[查看 PlantUML 源码](./diagrams/command/diagram.puml#L436)
...
- GUI 集成测试：聊天输入、Command Palette 和按钮都从同一目录发现并执行；
- 安全测试：输出不包含 token、密钥和未脱敏环境变量。

这套设计足以开发新命令：新增 Handler 与 Contributor Bean 后，启动安装器自动发现、校验，并使 GUI 的补全、Command Palette 和帮助同步生效。
...
- GUI 的聊天输入、Command Palette 和按钮调用都经过同一 Dispatcher 和 ExecutionGate；文本输入额外经过统一 Parser；
...
[查看 PlantUML 源码](./diagrams/command/diagram.puml#L464)
...
[查看 PlantUML 源码](./diagrams/command/diagram.puml#L490)
...
| GUI 集成测试 | 聊天输入、Command Palette 和按钮读取同一目录且无执行旁路 |
...
- 每个可执行命令均出现在统一帮助和补全中；
- GUI 的聊天输入、Command Palette 和按钮把命令调用统一交给 Dispatcher，业务行为位于 Handler；
- 所有生产调用经过 Dispatcher 和 ExecutionGate；
- 五种 execution mode 有穷尽测试；
- 已注册的 `/compact`、会话替换、`/reload` 和应用关闭命令符合各自策略；
...
| 展示层 | GUI 通过 Context 端口适配 | 业务 Handler 与 GUI 框架、WebSocket DTO 和运行容器解耦 |
...
| v1.12 | 2026-07-14 | 将面向用户的 `GUI Adapter` 统一更名为 `Agent UI`，明确结构化命令请求由服务端构造 Invocation，不再把 Adapter 作为独立架构组件 |
| v1.13 | 2026-07-14 | 将统一分发顺序图收敛为 SR 级 User、Agent UI、Agent Gateway、Agent Runtime 和 Model Manager 协作，所有元素使用 participant 并补齐 activate/deactivate 生命周期 |
