...
| Java 产品只提供 GUI | 只保留一个 GUI Adapter；聊天输入、命令面板和按钮复用同一执行管线 |
...
| 展示层 | GUI Adapter | 接收聊天输入、命令面板或按钮动作，提供交互、输出和刷新适配 |

关键依赖方向为“GUI → 命令核心 → Handler → 应用服务”。Handler 不依赖 Vue 组件、WebSocket DTO 或桌面框架类。
...
| GUI Adapter | 将 Interaction、Output 和刷新提示渲染到 GUI |
...
GUI 把 `select` 渲染为选择对话框、下拉面板或内嵌列表。Handler 通过同一方法表达“选择模型”意图，GUI Adapter 负责构造具体组件或 DTO。
